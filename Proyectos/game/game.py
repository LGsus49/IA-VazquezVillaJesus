...
def training(modo):
    ...
    if modo == "ad":
        ...
        model_rn.fit(X_train, y_train, epochs=50, batch_size=10, verbose=1)
...
def mostrar_menu():
    ...
    # Texto del menú con saltos de línea
    lineas_texto = [
        "Presiona 'M' para Manual",
        "'N' para Red",
        "'T' para Arbol",
        "'Q' para Salir"
    ]

    # Renderizar cada línea del texto
    y_offset = h // 2  # Posición inicial en Y
    espacio_entre_lineas = 30  # Espacio entre líneas
    for linea in lineas_texto:
        texto = fuente.render(linea, True, BLANCO)
        texto_rect = texto.get_rect(center=(w // 2, y_offset))
        ...
        for evento in pygame.event.get():
            ...
            if evento.type == pygame.KEYDOWN:
                if evento.key == pygame.K_n or evento.key == pygame.K_t:
                    selected_model = "ad" if evento.key == pygame.K_t else "rn"
