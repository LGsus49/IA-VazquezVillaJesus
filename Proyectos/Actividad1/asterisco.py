...
FONDO = (255, 255, 255)  # Color de fondo del grid (BLACK)
LINEAS = (128, 128, 128)  # Color de las líneas del grid (GREY)
COLOR_INICIO = (255, 165, 0)  # Color del nodo de inicio (TURQUOISE)
COLOR_FIN =  (64, 224, 208)  # Color del nodo de fin (RED)
COLOR_RUTA = (128, 0, 128) # Color de la ruta más corta (GREEN)
COLOR_PARED = (0,0,0)  # Color de los nodos que representan paredes (GREY)
COLOR_VISITADO = (0, 255, 0)  # Color de los nodos visitados durante la búsqueda (PURPLE)
COLOR_NODO = (128, 128, 128)  # Color de los nodos no visitados (GREY)

class Nodo:
    def __init__(self, fila, col, ancho, total_filas):  # Cambié _init_ a __init__
        self.fila = fila
        self.col = col
        self.x = fila * ancho
        self.y = col * ancho
    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_SPACE and start and end:  # Cambiar a pygame.K_SPACE
                    astar(lambda: dibujar(ventana, grid, filas, ancho), grid, start, end)


    pygame.quit()

if __name__ == "__main__":  # Uso correcto de __name__
